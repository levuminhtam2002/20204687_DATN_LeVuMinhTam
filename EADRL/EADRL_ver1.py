...
def update(self):
    ...
    for i in range(len(self.genes) - 1):
        ...
        offloading_ratio = env_.act[3]  # - tỷ lệ giảm tải
        ...
        if env_.sum_task_size == 0:  # - Nhiệm vụ tính toán của ue đã hoàn thành
            is_terminal = True
            reward = 0
            #self._ep_reward += 0
        elif env_.sum_task_size - task_size < 0:  # - Nhiệm vụ tính toán của bước cuối cùng không khớp với nhiệm vụ tính toán của ue
            #is_terminal = True
            ...
            # - Nếu vượt quá ranh giới, thì khoảng cách bay dist sẽ bằng không
            ...
            env_.e_battery_uav = env_.e_battery_uav - e_server  #- uav năng lượng còn lại
            ...
            delay = env_.com_delay(env_.loc_ue_list[ue_id], np.array([loc_uav_after_fly_x, loc_uav_after_fly_y]),
                                   0, task_size, block_flag)  # delay ~ Tính toán độ trễ
            reward = -delay
            env_.sum_task_size -= env_.task_list[ue_id]
            #self._ep_reward += -delay
            # self.reset2(delay, loc_uav_after_fly_x, loc_uav_after_fly_y, 0, task_size, ue_id)
        else:  # ~ Dung lượng pin hỗ trợ chuyến bay, các tác vụ tính toán hợp lý và các tác vụ tính toán có thể được tính toán trong dung lượng pin còn lại.
            delay = env_.com_delay(env_.loc_ue_list[ue_id], np.array([loc_uav_after_fly_x, loc_uav_after_fly_y]),
            ...
            #env_.loc_uav[0] = loc_uav_after_fly_x  #  - vị trí sau khi bay của uav
...
for i in range(MAX_EPISODES):
    ...
    while j < MAX_EP_STEPS:
        ...
        if i < MAX_EPISODES - 63:
            ...
            a = np.clip(np.random.normal(a, var), *a_bound)  # add randomness to action selection for exploration
            # print(a.shape)
            ...
            if offloading_ratio_change:
                a[3] = -1
            ddpg.store_transition(s_normal.state_normal(s), a, r, s_normal.state_normal(s_))  # Huấn luyện thưởng giảm 10 lần
